...
- @property {string} [key] - The key for the Hyperdrive, must start with the `pear://` prefix.
...
- @returns {Promise<void>}
  ...
  return
      ...
      this.logger.debug('Hyperdrive client created', { key: this.client.key?.toString('hex') })
      if (!this.swarm) {
      ...
      try {
        ...
        this.logger.debug('Hyperdrive client ready', { version: this.client.version })
      } catch (err) {
        throw new QvacErrorHyperdrive(ERR_CODES.DRIVE_NOT_READY)
      }

      this.swarm.join(this.client.discoveryKey, { client: true, server: false })
      this.logger.debug('Joined hyperswarm', { discoveryKey: this.client.discoveryKey.toString('hex') })
      ...
      if (this.opts.version) {
        ...
        this.logger.debug('Fetched blobs and determined latest version', { version })
      }
...
- @returns {Promise<Array<{key: string, cached?: boolean}>>} A list of files with their keys and cache status.
  ...
  this.logger.debug('Listing files in directory', { directoryPath, opts })
      ...
      const filesStream = this.drive.list(directoryPath, { ...opts, recursive: true })
      ...
      this.logger.debug('Directory listing complete', { directory: directoryPath, count: files.length })
      return files

  } catch (err) {
  this.logger.error('Failed to list directory', { directoryPath, error: err })
...
- Returns true if downloaded and false if files were already downloaded before
- @param {string} path - The path to download the file from.
- @param {ProgressReport} [progressReport] - Optional progress reporter instance.
- @returns {Promise<boolean>} True if downloaded, false if already cached.
  \*/
  async download (path = '/', progressReport = null) {
  try {
  this.logger.debug('download called', { path })
  this.\_checkDrive()

      if (await this.cached(path)) {
        this.logger.debug('Path already cached, skipping download', { path })
        return false
      }

      const files = await this._getFilesToDownload(path)
      this.logger.debug('Files to download', { files })

      const trackers = []
      for (const file of files) {
        const tracker = new HyperdriveProgressTracker(this.drive, file, this.logger, progressReport)
        tracker.downloadStart()
          .then(() => this.logger.debug('Download completed', { file }))
          .catch((err) => this.logger.error('Download failed', { file, error: err }))
  ...
  throw new QvacErrorHyperdrive(ERR_CODES.DOWNLOAD_FAILED, `${path}: ${err.message}`)
  }
  }
