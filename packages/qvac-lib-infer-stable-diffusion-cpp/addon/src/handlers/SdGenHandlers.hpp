...
struct SdGenConfig {
  ...
  float imgCfgScale = -1.0f;      // img_cfg  — image guidance for img2img/inpaint; -1 = cfgScale
  ...
  int   clipSkip = -1;            // skip last N CLIP encoder layers (SD1.x / SD2.x); -1 = auto
 ...
 */
using SdGenHandlerFn   = std::function<void(SdGenConfig&, const picojson::value&)>;
