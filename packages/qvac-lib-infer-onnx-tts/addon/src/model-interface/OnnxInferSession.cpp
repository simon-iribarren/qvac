...
OnnxInferSession::OnnxInferSession(const std::string& modelPath) {
  ...
  for (size_t i = 0; i < session_->GetInputCount(); i++) {
    const Ort::AllocatedStringPtr inputName = session_->GetInputNameAllocated(i, allocator_);
    inputNames_.push_back(std::string(inputName.get()));
  }

  // collect output names
  for (size_t i = 0; i < session_->GetOutputCount(); i++) {
    Ort::AllocatedStringPtr outputName = session_->GetOutputNameAllocated(i, allocator_);
...
void OnnxInferSession::initInputTensors(const std::vector<std::vector<int64_t>>& inputShapes) {
  inputTensors_.clear();
  inputTensorsValues_.clear();

  for (size_t i = 0; i < session_->GetInputCount(); i++) {
    const Ort::TypeInfo inputTypeInfo = session_->GetInputTypeInfo(i);
    const Ort::ConstTensorTypeAndShapeInfo inputShapeInfo = inputTypeInfo.GetTensorTypeAndShapeInfo();

    std::vector<int64_t> inputShape = inputShapes[i];
    ONNXTensorElementDataType onnxType = inputShapeInfo.GetElementType();

    Ort::Value inputValue = Ort::Value::CreateTensor(allocator_, inputShape.data(), inputShape.size(), onnxType);
