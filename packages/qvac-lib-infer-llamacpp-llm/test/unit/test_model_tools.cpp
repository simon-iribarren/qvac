...
class ModelToolsTest : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(ModelToolsTest, CacheEnabledWithToolMessage) {
...
TEST_F(ModelToolsTest, CacheEnabledWithToolMessageToolsAtEndFalse) {
  if (!isQwen3ModelPath(test_model_path)) {
    GTEST_SKIP() << "Test model not found";
  }
...
TEST_F(ModelToolsTest, MultiTurnWithToolsAndCache) {
  if (!isQwen3ModelPath(test_model_path)) {
    GTEST_SKIP() << "Test model not found";
  }
