...
setup_file() {
  ...
  # Write qvac.config.json with the 3 smallest models
...
CONF
  ...
  node -e "
    ...
    require('fs').writeFileSync('$FILE_TMPDIR/silence.wav', b);
  "
  ...
  # Wait for all 3 models to be loaded (poll /v1/models)
  local max_wait=300  # 5 minutes for model download+load
  local elapsed=0
...
@test "chat: SSE stream returns valid chunks" {
  ...
  # Last data chunk (before [DONE]) should have a finish_reason
  local last_chunk
  ...
  # Should contain at least one content token
  local content_count
...
@test "transcriptions: response_format=text returns plain text" {
  ...
  # Plain text response should NOT be valid JSON (it's just the text)
