...
void applyModelPathFromJs(js::Object &jsObject, js_env_t *env,
...
void applyModelTypeFromJs(js::Object &jsObject, js_env_t *env,
                          ParakeetConfig &config) {
  auto modelTypeOpt =
...
auto JSAdapter::loadFromJSObject(js::Object jsObject, js_env_t *env)
    -> ParakeetConfig {
  ParakeetConfig config;
  applyModelPathFromJs(jsObject, env, config);
  applyModelTypeFromJs(jsObject, env, config);
...
void setMapEntryFromJsValue(js_env_t *env, const std::string &key,
                            js_value_t *value,
...
void JSAdapter::loadMap(js::Object jsObject, js_env_t *env,
  ...
  for (uint32_t i = 0; i < length; ++i) {
    ...
    setMapEntryFromJsValue(env, key, value, output);
  }
}
